...
if __name__ =="__main__":
    ...
    df_user_id = df['userId'].unique().tolist()
    user_prefer_dict = {}
    for uid in df_user_id:
        user_prefer = []
    ...
    df_user_prefer = pd.DataFrame(0, index=df_user_id, columns=genreslist)
    for uid in df_user_id:
        user_prefer = []
        filter_user = df[df['userId']==uid]
        for user_genres in filter_user['genres_list']:
...
model = node2vec.fit(window=20, min_count=1, batch_words=4,sg=1)
...
search_list = ['Children','4993','8368']
...
df_title = df_movies[['movieId','title']].values.tolist()
alien_list = [result_list for result_list in df_title if 'alien' in result_list[1].lower()]
pprint(alien_list)